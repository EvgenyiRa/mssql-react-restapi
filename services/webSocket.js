        ...
        if (dataP.type==='dataUpdate') {
          ...
          if ((!!dataP.data)
            ...
            try {
              ...
              context.params=[userCntlID,parseInt(dataP.data.data.timeAll/1000),dataP.date,(dataP.data.data.access)?1:0];
              context.sql=`INSERT INTO REP_USR_CNTRL_SYS_STATE
                                      (REP_USR_CNTRL_ID,TIME_ALL,DATE,ACCESS)
                                VALUES (?,?,STR_TO_DATE(?, '%d-%m-%Y'),?) AS new
              ...
              for (var key in dataP.data.data.winsActiveSum) {
                ...
                context={};
                context.params=[userCntlID,dataP.date,key,prcClient.pid,(prcClient.access)?1:0,prcClient.lastTimeProcess,prcClient.timeAll,parseInt(prcClient.timeAllDelta/1000),prcClient.timeAllUser];
              ...
              if (!!dataP.data.data.browser) {
                  for (var key in dataP.data.data.browser) {
                    ...
                    context={};
                    context.params=[userCntlID,dataP.date,key,parseInt(brwrOneHost.timeAll/1000)];
